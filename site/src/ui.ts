...
export function mountCalculator(root: HTMLElement, deps: CalculatorDependencies): () => void {
  ...
  // 이름은 게임사가 정한 것을 쓴다(`data/locale_text.json`) — 한국어면 그대로 나온다.
  resolveDisplayName = tName;
    ...
    if (fiveDeckMode) {
      ...
      if (shared.length > 0) {
        deckNote.replaceChildren(
          ...
          createText('span', shared.map(([name, ids]) => `${resolveDisplayName(name)}(${t('덱 {n}', { n: ids.join('·') })})`).join(', ')),
  ...
  const applyDeckCopy = () => {
    ...
    saveState();
    renderDeckTabs();
    status.textContent = t('{from}덱을(를) {to}에 복사했습니다.', {
      from: source.id, to: targets.map((id) => t('덱 {n}', { n: id })).join(' · '),
    });
  };
  ...
  const makeDropTarget = (card: HTMLElement, index: number) => {
    ...
    card.addEventListener('drop', (event) => {
      ...
      if (already >= 0 && already !== index) {
        showErrors([t('{name}은(는) 이미 {n}번 칸에 있습니다.', { name: resolveDisplayName(name), n: already + 1 })]);
        return;
      }
  ...
  const copyFromControl = (name: string): HTMLElement => {
    ...
    apply.addEventListener('click', () => {
      ...
      if (carried.length === 0) {
        status.textContent = t('{name}에게는 베껴올 육성값이 없습니다.', { name: resolveDisplayName(pick.value) });
        return;
      }
      deck.characters[name] = next;
      saveState();
      renderSquad();
      status.textContent = t('{from}의 {what}을(를) {to}에게 베꼈습니다.', {
        from: resolveDisplayName(pick.value), what: carried.join(' · '), to: resolveDisplayName(name),
      });
    });
  ...
  const renderSquad = () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      ...
      if (char?.image) {
        ...
        image.alt = t('{name} 초상화', { name: resolveDisplayName(char.name) });
        image.loading = 'lazy';
        portrait.append(image);
      }
      ...
      choose.append(createText('strong', char ? resolveDisplayName(char.name) : '빈 칸'));
  ...
  const validateCharacterValues = (deck: DeckState): string[] => {
    ...
    for (const [name, custom] of Object.entries(deck.characters)) {
      ...
      if (custom.growthStage !== undefined && (
        ...
        messages.push(
          `덱 ${deck.id} · ${resolveDisplayName(name)}: 돌파 단계는 0~${characterDefaults?.maxGrowthStage ?? 0} 정수여야 합니다.`,
        );
      }
      if (custom.skillLevels) {
        ...
        if (!hasExactKeys || values.some((value) => !Number.isInteger(value) || value < 1 || value > 10)) {
          messages.push(`덱 ${deck.id} · ${resolveDisplayName(name)}: 스킬 레벨은 1~10 정수여야 합니다.`);
        } else if (characterDefaults?.skillLevelsLocked
          && values.some((value) => value !== 10)) {
          messages.push(`덱 ${deck.id} · ${resolveDisplayName(name)}: 수치 미공개 캐릭터는 스킬 Lv10만 사용할 수 있습니다.`);
        }
      }
      ...
      if (custom.cube && (!settings.cubes[custom.cube.name] || !Number.isInteger(custom.cube.level)
        ...
        messages.push(`덱 ${deck.id} · ${resolveDisplayName(name)}: 큐브 설정을 확인해 주세요.`);
      }
      if (custom.weaponModeSwapAt !== undefined && (
        ...
        messages.push(`덱 ${deck.id} · ${resolveDisplayName(name)}: 저격 모드 변경 시점은 0~180초여야 합니다.`);
      }
  ...
  const renderPresets = () => {
    ...
    for (const preset of presets) {
      ...
      load.title = t('{at} 저장 · 눌러서 불러오기', { at: preset.at.slice(0, 10) });
    ...
    for (const entry of calcHistory) {
      ...
      for (const deck of entry.decks) {
        row.append(createText(
          'p',
          `덱 ${deck.id} · ${formatDamage(deck.total)} — ${deck.squad.map(resolveDisplayName).join(', ') || t('빈 덱')}`,
          'history-deck',
        ));
      }
  ...
  const applyShareText = (text: string, scope: ShareScope = shareScope) => {
    try {
      ...
      const missing = skipped.length > 0
        ? ` · ${t('목록에 없는 니케 {n}명 제외', { n: skipped.length })}(${skipped.slice(0, 3).map(resolveDisplayName).join(', ')}${skipped.length > 3 ? '…' : ''})`
        : '';
      ...
      if (scope === 'all') {
        ...
        showShareMsg(`코드에 덱이 ${carried}개 들어 있어 첫 덱을 덱 ${landed}에 넣었습니다`
      ...
      const meta: ReportMeta = {
        ...
        displayNames: Object.fromEntries(
          [...new Set(names)].filter(Boolean)
            .map((name) => [name, resolveDisplayName(name)]),
        ),
      };
  ...
  const renderBatchResult = (batch: BatchResult) => {
    ...
    for (const entry of batch.decks) {
      ...
      for (const name of entry.request.squad) {
        ...
        const shown = resolveDisplayName(name);
        if (shown !== name) timelineNames[name] = shown;
      }
  ...
  function sortRoster(list: CharacterMeta[]): void {
    ...
    // 늘어서면 「이름순」이라는 말이 거짓이 된다. 고른 말에 따라 순서도 따라 바뀐다.
  ...
  const renderRosterGrid = () => {
    ...
    const all = [...catalogByName.values()]
      .sort((a, b) => resolveDisplayName(a.name).localeCompare(resolveDisplayName(b.name)));
  ...
  const pickCharacter = (name: string, targetSlot = activeSlot) => {
    ...
    if (catalogByName.get(name)?.preview) {
      status.textContent = `${resolveDisplayName(name)}은(는) 아직 (임시) 등록입니다 — 스킬이 공개되지 않아 `
  ...
  const updatePickerTarget = () => {
    ...
    rosterDesc.textContent = current
      ? t('{n}번 칸을 {name} 대신 채웁니다 · {filled}/5명', {
        n: activeSlot + 1, name: resolveDisplayName(current), filled,
      })
      : t('{n}번 빈 칸을 채웁니다 · {filled}/5명', { n: activeSlot + 1, filled });
  };
  ...
  const renderExcludeChips = () => {
    ...
    for (const name of enikkExcluded) {
      ...
      chip.title = t('{name} 제외 해제', { name: resolveDisplayName(name) });
  ...
  const renderEnikk = (data: EnikkImport) => {
    ...
    const drawCards = () => {
      ...
      for (const [n, deck] of player.decks.entries()) {
        ...
        if (blocked && deck.usable) {
          row.title = `${t('제외한 니케가 껴 있습니다')} — ${deck.squad.filter((n) => enikkExcluded.includes(n)).map(resolveDisplayName).join(', ')}`;
        }
    ...
    const drawPager = (box: HTMLElement) => {
      ...
      box.append(createText('span', t('{all}쪽 중 {n}쪽', { all: pages, n: enikkPage + 1 }), 'enikk-page-info'));
    };
