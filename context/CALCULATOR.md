...
#### fork 고유 `enemy["boss_phases"]` — 같은 스케줄러 위로 폈다

이 repo에는 Boss Maker·연합 다섯 왕이 쓰는 **평평한 시간 창** 여섯 종이 따로 있었다.
같은 일을 하는 두 구현을 나란히 두지 않기 위해 `phases_to_patterns()`가 편다 ——
**입력 형식은 그대로**라 유저가 저장해 둔 판·분享 코드는 손댈 필요가 없다.

| `boss_phases` | 패턴 | 옮기면서 지킨 것 |
|---|---|---|
| `core` | `core` | 창 안에서는 적의 `core_px`, **밖에서는 0** → 기본값을 0으로 내리고 창마다 연다 |
| `parts` | `parts` + `emit_end` | 안 깨지는 표적(hp 0) 하나. 부위 파괴는 **창이 끝날 때** 창마다 한 번 |
| `optimal_range` | `move` | 창 안은 정적 목록과 **합집합**, 겹치면 **먼저 시작한 창이 이긴다** — `move`는 반대(나중이 이김)라 **경계로 잘라 겹치지 않게 편다** |
| `immune` | `immune` | **딜 전부 차단**(유저 확인 2026-09-19). 上游 `vanish`는 평타만 막으므로 **다른 종류**다 |
| `pierce_gate` | `pierce_gate` | 관통 딜만 통과. 上游에 대응이 없어 fork가 종류를 추가했다 |
| `element_gate` | `shield` | 판정 기준인 적 코드를 `code`로 넣는다. 적 코드가 없으면 **거절한다** |

**딜 게이트는 히트 자신의 시각(`ev.t`)으로 판정한다.** 스킬은 발동 다음 프레임에
수거되기도 해서, 수거 프레임으로 재면 구간 안에서 난 피해가 밖으로 샌다. 上游는 프레임
상태로 재지만 이 fork는 `boss_phases` 시절부터 이 규약이었고(그 회귀가
`test_union_boss_phases.py`에 있다), 옳은 쪽이라 통일했다. 그래서 게이트 패턴 목록은
**전투 내내 고정**이고 구간 판정만 `_Run.open_at()`이 한다 — 「지금 열려 있는 것」만 보면
늦게 수거된 히트가 판정에서 통째로 빠진다.

> 브라우저 상세 설정의 **족자·속저**(`immune_windows`·`element_windows`)는 별개 기능이고
> **수거 프레임 판정을 유지한다**(`_gate()`). 종전부터 그랬고 바꿀 근거가 없다.

등가의 기준은 `calculator/test_boss_phase_numbers.py` — 통일 **전** 구현에서 뽑은
13가지 조합의 총딜·캐릭터별 딜·히트 수를 박아 뒀다.
