...
`e028cbc`（2026-08-25）。那筆 commit 自己寫著修完是「阿妮斯 40.2% · 梅登 35.7%」。
今天梅登 35.6%（幾乎沒動），**阿妮斯掉到 35.0%** —— 是這個 fork 歷史裡的回歸，可以用
`git bisect` 定位。測試已改成只釘現在成立的那半，阿妮斯那半標成 `expectedFailure`。
